...
import { fetchCrimeData, CrimeData } from "../../api/crimeDataApi";
...
const DisplayResultsView = ({
  ...
  const handleSearch = useCallback(
    async (postcode: string): Promise<CrimeData[] | void> => {
      try {
  ...
  useEffect(() => {
    if (postcode) {
      Promise.all(postcode.split(",").map((postcode) => handleSearch(postcode)))
        .then((data) => {
          setSelectedCrimeData(data.filter((data) => data).flat());
        })
        .catch((error) => {
  ...
  return (
    <div>
      <Link
        className="link ml-20 self-start text-pink-500 underline mt-10"
        to="/"
      >
        Go back home
      </Link>
